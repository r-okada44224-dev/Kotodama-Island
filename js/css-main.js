...
// ----- セーブデータ（localStorage） -----
const SAVE_KEY = 'kotodama_css_v1';
function loadSave() {
  try {
...
// 現在のモードのクエスト配列
function questSet() {
  return state.mode === 'review' ? CSS_REVIEW_QUESTS : CSS_QUESTS;
}
...
function refreshTitleButtons() {
  ...
  if (cleared > 0 && cleared < CSS_QUESTS.length) {
  ...
  if (cleared >= CSS_QUESTS.length) {
    const reviewCleared = save.clearedReview || 0;
    trainingButton.textContent = reviewCleared >= CSS_REVIEW_QUESTS.length
      ? '🔥 とっくんモード（もういちど）'
      : '🔥 とっくんモード';
...
function continueGame() {
  ensureAudio();
  playClickSound();
  const cleared = Math.min(loadSave().cleared || 0, CSS_QUESTS.length - 1);
  titleScreen.classList.remove('active');
  gameScreen.classList.add('active');
  state.mode = 'main';
...
function startTraining() {
  ...
  if (startIndex >= CSS_REVIEW_QUESTS.length) startIndex = 0;
  ...
  state.mode = 'review';
  state.level = CSS_QUESTS.length + 1;
